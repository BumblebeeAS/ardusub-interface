...
#
# Launch the multivehicle_sim container with rocker (NVIDIA GPU + X11 + joystick + host net).
#
# Usage: ./run.bash [-c|-s|-t|-x|-h] <image>
#   <image>   image to run, e.g. multivehicle_sim:jazzy
...
Help()
{
   echo "Launch the multivehicle_sim container with rocker."
   echo
...
ROCKER_ARGS="--devices /dev/dri $JOY --dev-helpers --nvidia --x11 --git --volume ${WORKSPACE_DIR}:/root/HOST/mvsim_ws --network=host"
