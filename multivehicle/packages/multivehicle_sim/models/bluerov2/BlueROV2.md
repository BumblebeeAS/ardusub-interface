...
> **Not available inside `multivehicle_sim:jazzy`.** The image ships only the
> `ardusub` binary (`/usr/local/bin/ardusub`), not the ArduPilot source tree, so
> `Tools/autotest/sim_vehicle.py` and MAVProxy are absent. In the container use
> `ros2 launch multivehicle_sim bluerov.launch.py`, which execs `ardusub`
> directly (see `multivehicle_interface/launch/ardusub_interface.launch.py`).
> The command below is for a host checkout of ArduPilot.
